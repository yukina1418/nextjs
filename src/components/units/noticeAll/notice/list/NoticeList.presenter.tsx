  ...
  return (
    <S.Wrapper>
      ...
      <S.ReviewMenuBox>
        ...
        <S.ReviewMenu onClick={props.onClickNoticePromotion}>
          프로모션
        </S.ReviewMenu>
        <S.ReviewMenu onClick={props.onClickNoticTaster}>
          시식단 모집
        </S.ReviewMenu>
      </S.ReviewMenuBox>
        ...
        {props.data?.fetchNoticeCategoryPick?.map((el: any, index: any) => (
          <>
            <S.NoticeTd key={el.noticeId}>
              <S.NoticeTdCategory>
                {el?.subCategory?.subCategoryName === "TASTING" &&
                  "시식단 리뷰"}
              ...
              <S.NoticeTdTitle
                id={el.noticeId}
                onClick={props.onClickMoveNoticeDetail}
              >
                {el.noticeTitle}
              </S.NoticeTdTitle>
              <S.NoticeTdCreateAt>{getDateDot(el.createAt)}</S.NoticeTdCreateAt>
              <S.NoticeTdHits>{el.noticeHit}</S.NoticeTdHits>
            </S.NoticeTd>
            <S.NoticeTdHr />
          </>
        ))}
